...
mapfile -t SHIELDS < <( yq -r '.include[].shield' $HOST_CONFIG_DIR/build.yaml )
...
compile_board() {
    ...
    [[ $MULTITHREAD = "true" ]] || echo -en "\n$(tput setaf 2)Building $2 ($1)... $(tput sgr0)"
    [[ $MULTITHREAD = "true" ]] && echo -e "$(tput setaf 2)Building $2 ($1)... $(tput sgr0)"
    $DOCKER_PREFIX west -v build -d "build/$BUILD_DIR" -b $1 $WEST_OPTS \
        -- -DZMK_CONFIG="$CONFIG_DIR" -DZMK_EXTRA_MODULES="$MODULES" -DSHIELD="$2" -Wno-dev >"$LOGFILE" 2>&1
    if [[ $? -eq 0 ]]; then
        ...
        OUTPUT="$OUTPUT_DIR/$1_$2-zmk.$TYPE"
